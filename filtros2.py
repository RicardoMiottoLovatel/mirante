...
def classificar_porte(pop):
    if pd.isna(pop):
        ...
        return "Até 5 mil habitantes"
    elif pop <= 20000:
        return "Entre 5 mil e 20 mil habitantes"
    elif pop <= 100000:
        return "Entre 20 mil e 100 mil habitantes"
    elif pop <= 500000:
        return "Entre 100 mil e 500 mil habitantes"
    else:
        return "Acima de 500 mil habitantes"
...
df["pop_censo_2022"] = pd.to_numeric(df["pop_censo_2022"],errors="coerce").astype(int)

df["faixa_populacao"] = df["pop_censo_2022"].apply(classificar_porte)


faixas = [
    "Até 5 mil habitantes",
    "Entre 5 mil e 20 mil habitantes",
...
selecionar_todos = st.sidebar.toggle("Selecione todos")
...
#    options=ufs,
#    default=[],
#    placeholder="Selecione um ou mais UF"
#)
...
estado_selecionado = st.sidebar.multiselect(
    ...
    options=estados,
    default=[],
    placeholder="Selecione uma ou mais UF"
)

# 4. Porte da população
portes = sorted(df["Porte_pop_2022_label"].dropna().unique())
porte_selecionado = st.sidebar.multiselect(
    "Porte Populacional SUAS",
    options=portes,
    default=[]
)

# 5. Faixa de população (slider)
pop_numerica = pd.to_numeric(df["pop_censo_2022"], errors='coerce')

pop_min = pop_numerica.min()
pop_max = pop_numerica.max()
...
pop_numerica = pd.to_numeric(df_filtrado["pop_censo_2022"], errors='coerce')

df_filtrado = df_filtrado[
    (pop_numerica  >= faixa_pop[0]) &
    (pop_numerica  <= faixa_pop[1])
]
...
df_filtrado["pop_censo_2022_fmt"] = df_filtrado["pop_censo_2022"].apply(lambda x: formato_br(x))
...
df_filtrado["pop_censo_2022"] = pd.to_numeric(
...
st.dataframe(
    ...
    column_config={
        ...
        "Porte_pop_2022_label": st.column_config.TextColumn("Porte Município"),
       
        "pop_censo_2022_fmt": st.column_config.TextColumn("População 2022"),

        "pop_censo_2022": st.column_config.NumberColumn("População Original"),  # Esconde a coluna original de população
        "Porte_pop_2022": None,
    }
)
